...
import { Briefcase, Loader2, CheckCircle2, AlertTriangle, ArrowRight, RefreshCw } from "lucide-react";
import type { PositionResearch } from "@/types";

export default function PositionResearchPage() {
  const { companyName, jobDescription } = useJobStore();
  const { positionResearch: result, setPositionResearch } = useResultsStore();
  ...
  async function handleResearch() {
    ...
    if (!res.ok) {
      ...
      setPositionResearch(data as PositionResearch);
    }
    setLoading(false);
  }
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-10">
        {!result && !loading && (
          <div className="flex h-52 items-center justify-center rounded-lg border border-dashed border-slate-200 text-[13px] text-slate-400">
            Paste a job description above and click Analyse position.
          </div>
        )}
